...
mongoose.connect("mongodb://localhost:27017/integral",function(error){
    ...
    app.get("/musteri_rapor" ,AccountController.sessionCheck ,function(req, res){
        ...
        res.render('pages/musteri_rapor', {layout : false, session : req.session});   
    });
    app.get("/urun_rapor" ,AccountController.sessionCheck ,function(req, res){
        req.session.currentPage = "/urun_rapor";
        req.session.pageLabel = "raporlar";
        res.render('pages/urun_rapor', {layout : false, session : req.session});   
    });
    // Raporlar End
            ...
            tds.listAll(req.session.user.firmCode,function(stateTasks, responseTasks){
                ...
                if(req.session.user.firmCode == 'administrator'){
                    userService.listAll(function(stateUser, responseUsers){
                        ...
                        res.render('pages/kullanici_tanimi', {layout : false, session : req.session, roles: responseRoles, tasks : responseTasks, users : responseUsers}); 
                    });   
                }
                else{
    ...
    app.get("/rol_tanimi" ,AccountController.sessionCheck ,function(req, res){
        ...
        ads.listAll(req.session.user.firmCode,function(state,response){
            if(!state){
             ...
             res.render('pages/rol_tanimi', {layout : false, session : req.session,acts:response}); 
        });  
    });
    //end
    // görev tanımları "abuzer" 27.02
    app.get("/gorev_tanimi" ,AccountController.sessionCheck ,function(req, res){
        ...
        res.render('pages/gorev_tanimi', {layout : false, session : req.session, tasks:response }); 
        });
    });
    //end
    ...
    app.get("/vade_tanimlari" ,AccountController.sessionCheck ,function(req, res){
        ...
        crs.listAll(req.session.user.firmCode, function(state,response){
            ...
            res.render('pages/vade_tanimlari', {layout : false, session : req.session, credits:response});
        });  
    });
    app.get("/il_ilce_tanimlari" ,AccountController.sessionCheck ,function(req, res){
        ...
        cityservice.listAll(function(state,response){
            ...
            res.render('pages/il_ilce_tanimlari', {layout : false, session : req.session,citys:response});  
        }); 
    });
    app.get("/firma_tanimlari" ,AccountController.sessionCheck ,function(req, res){
        ...
        firmService.listAll(function(state, response){
            ...
            res.render('pages/firma_tanimlari', {layout : false, session : req.session, firms : response});  
        }); 
    });
    ...
    app.get("/teklif_konu_tanimlari" ,AccountController.sessionCheck ,function(req, res){
        ...
        ots.listAll(req.session.user.firmCode,function(state,response){
            if(!state){
             ...
             res.render('pages/teklif_konu_tanimlari', {layout : false, session : req.session,offerTopics : response});   
        });
    });
    //end
    //teklif durumu "abuzer" 03.03 start
    app.get("/teklif_durum_tanimlari" ,AccountController.sessionCheck ,function(req, res){
        ...
        oss.listAll(req.session.user.firmCode,function(state,response){
          ...
          res.render('pages/teklif_durum_tanimlari', {layout : false, session : req.session,offerStatuses : response});   
         });
    });
    app.get("/kaybetme_nedenleri" ,AccountController.sessionCheck ,function(req, res){
        ...
        lrs.listAll(req.session.user.firmCode,function(state,response){
            if(!state){
             ...
             res.render('pages/kaybetme_nedenleri', {layout : false, session : req.session,losingReasons :response});   
        });
    });
    //ürünler 
    ...
    app.get("/urun_fiyat_tanimi" ,AccountController.sessionCheck ,function(req, res){
        ...
        productService.listAll(req.session.user.firmCode, function(stateProduct, responseProduct){
            ...
            res.render('pages/urun_fiyat_tanimi', {layout : false, session : req.session, products : responseProduct});   
        });
    });
    ...
    app.get("/urun_grup_tanimi" ,AccountController.sessionCheck ,function(req, res){
        ...
        pds.listAll(req.session.user.firmCode, function(state,response){
            ...
            res.render('pages/urun_grup_tanimi', {layout : false, session : req.session, productGroups :response});
        });   
    });
    //start
    app.get("/urun_listeleme" ,AccountController.sessionCheck ,function(req, res){
        ...
        productService.listAll(req.session.user.firmCode, function(state, response){
            ...
            pds.listAll(req.session.user.firmCode, function(statePGroup, responsePGroup){
                ...
                res.render('pages/urun_listeleme', {layout : false, session : req.session, products : response, productGroup : responsePGroup}); 
            });            
        });
    ...
    app.get("/kaplama_sekli" ,AccountController.sessionCheck ,function(req, res){
        ...
        cts.listAll(req.session.user.firmCode,function(state,response){
            ...
            res.render('pages/kaplama_sekli', {layout : false, session : req.session, coverTypes:response}); 
        });     
    });
    //end
        ...
        custservice.listAll(req.session.user.firmCode,function(state,response){
            ...
            cgs.listAll(req.session.user.firmCode,function(stateCustGroup,responseCustGroup){
             ...
             userService.listCustomerAgent(req.session.user.firmCode,function(stateCustAgnt,responseCustAgnt){
                ...
                res.render('pages/musteri_listesi', {layout : false, session : req.session, customers : response,custGroups : responseCustGroup,custAgents : responseCustAgnt});
               });
            });
        });   
    });
    ...
    app.get("/acik_teklifler" ,AccountController.sessionCheck ,function(req, res){
        ...
        req.session.pageLabel = "/teklifYonetim";
        offerService.search({'status.offerCase' : 'acik_teklifler', 'firmCode' : req.session.user.firmCode},function(stateOffers,responseOffers){
             if(!stateOffers){
        ...
        userService.listFirmUser(req.session.user.firmCode, function(stateUser, responseUsers){
            ...
            res.render('pages/acik_teklifler', {layout : false, session : req.session,offers : responseOffers,orderStatuses : responseOfferStatus,losingReasons : responseLosingReason, users : responseUsers});
        });
        });
      });
     });
    ...
    app.get("/kazanilmis" ,AccountController.sessionCheck ,function(req, res){
        ...
        offerService.search({'status.offerCase' : 'kazanilmis', 'firmCode' : req.session.user.firmCode},function(stateOffers,responseOffers){
            if(!stateOffers){
                console.error(responseOffers);
                res.render("/pages/index",{layout : false, session : req.session});
                return;
            }
             res.render('pages/kazanilmis', {layout : false, session : req.session,offers : responseOffers}); 
        }); 
    });
    app.get("/kaybedilmis" ,AccountController.sessionCheck ,function(req, res){
        req.session.currentPage = "/kaybedilmis";
        req.session.pageLabel = "/teklifYonetim/kapali";
        offerService.search({'status.offerCase' : 'kaybedilmis', 'firmCode' : req.session.user.firmCode},function(stateOffers,responseOffers){
            if(!stateOffers){
                console.error(responseOffers);
                res.render("/pages/index",{layout : false, session : req.session});
                return;
            }
        res.render('pages/kaybedilmis', {layout : false, session : req.session,offers : responseOffers});   
    });
  });
    //Teklif End
    ...
    app.get("/is_emri/onay_bekleyen" ,AccountController.sessionCheck ,function(req, res){
        ...
        offerService.search({"status.job" : "onay_bekleyen", "firmCode" : req.session.user.firmCode},function(stateOffers,responseOffers){
            if(!stateOffers){
                console.error(responseOffers);
                res.render("/pages/index",{layout : false, session : req.session});
                return;
            }
            res.render('pages/is_emri_onay_bekleyen', {layout : false, session : req.session,offers:responseOffers});
        });   
    });
    //Onay_Bekleyen End
    ...
    app.get("/is_emri/devam_eden" ,AccountController.sessionCheck ,function(req, res){
        ...
        req.session.pageLabel = "isEmri";
        offerService.search({"status.job" : "devam_eden", "firmCode" : req.session.user.firmCode},function(stateOffers,responseOffers){
            if(!stateOffers){
                console.error(responseOffers);
                res.render("/pages/index",{layout : false, session : req.session});
                return;
            }
            res.render('pages/is_emri_devam_eden', {layout : false, session : req.session,offers:responseOffers});
        });   
    });
    //Devam_eden End
    ...
    app.get("/is_emri/tamamlanmis" ,AccountController.sessionCheck ,function(req, res){
        ...
        req.session.pageLabel = "isEmri";
        offerService.search({"status.job" : "tamamlanmis", "firmCode" : req.session.user.firmCode},function(stateOffers,responseOffers){
            if(!stateOffers){
                console.error(responseOffers);
                res.render("/pages/index",{layout : false, session : req.session});
                return;
            }
            res.render('pages/is_emri_tamamlanmis', {layout : false, session : req.session,offers:responseOffers});
        });   
    });
    ...
    app.get("/hesap_bilgileri" ,AccountController.sessionCheck ,function(req, res){
        ...
        custservice.listAll(req.session.user.firmCode, function(state, response){
            ...
            res.render('pages/hesap_bilgileri', {layout : false, session : req.session, customers : response});
        })
    });
